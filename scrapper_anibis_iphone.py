...
def get_page(liste_url):
    ...
    try:
        ...
        for page_nb in range(1, 20):
            page_url = f"{liste_url}{page_nb}"
            try:
                driver.get(page_url)
                rnd = random.randint(4, 20)
                time.sleep(rnd)
...
def save_pages(pages, dir):
    ...
    for page_nb, page in enumerate(pages):
        with open(f"{dir}/page_{page_nb}.html", "wb") as f_out:
            f_out.write(page)
            logging.info(f"SAVE_PAGES: {dir}/page_{page_nb}.html")
...
def parse_page(page):
    ...
    for div in soup.find_all("h6", class_="MuiTypography-root MuiTypography-h6 mui-style-2uyuut"):
...
def get_content_page(url):
    ...
    try:
        driver = webdriver.Firefox(options=options)
        driver.set_page_load_timeout(30)  # Set timeout for page load
        page_url = f"https://www.anibis.ch{url}"
        page_id = re.search(r'/(\d+)$', url).group(1)
        driver.get(page_url)
        rnd = random.randint(7, 21)
        time.sleep(rnd)
        page = driver.page_source.encode("utf-8")
        logging.info(f"GET_PAGE_CONTENT: {page_url}")
        os.makedirs("content_anibis_iphone", exist_ok=True)
        with open(f"content_anibis_iphone/page_content_{page_id}.html", "wb") as f_out:
            f_out.write(page)
            logging.info(f"SAVE_CONTENT_PAGES: content_anibis_iphone/page_content_{page_id}.html")
    except WebDriverException as e:
...
def parse_content(dir):
    ...
    for pages_path in pages_paths:
        ...
        with open(os.path.join(dir, pages_path), "rb") as f_in:
            ...
            soup = BeautifulSoup(page, "html.parser")
            logging.info("PARSE_PAGE -> ")
            for div in soup.find_all("div", class_="MuiBox-root mui-style-1ftdlj9"):
                titre = soup.find('h1', class_='MuiTypography-root MuiTypography-h5 mui-style-1jixzed').get_text(strip=True) if soup.find('h1', class_='MuiTypography-root MuiTypography-h5 mui-style-1jixzed') else None
                contenu = soup.find('div', class_="MuiBox-root mui-style-znb5ut").get_text(strip=True) if soup.find('div', class_="MuiBox-root mui-style-znb5ut") else None
                # Extraction du prix
                p = soup.find('span', class_="MuiTypography-root MuiTypography-body1 ecqlgla1 mui-style-1t1wil6").get_text(strip=True) if soup.find('span', class_="MuiTypography-root MuiTypography-body1 ecqlgla1 mui-style-1t1wil6") else None
                prix_match = re.search(r'(\d+)', p) if p else None
                prix = prix_match.group(1) if prix_match else None
                # Extraction du pseudo et du lien href
                pseudo = soup.find('h4', class_='MuiTypography-root MuiTypography-h6 mui-style-eeciap').get_text(strip=True) if soup.find('h4', class_='MuiTypography-root MuiTypography-h6 mui-style-eeciap') else None
                pseudo_tag = soup.find('a', class_='MuiButtonBase-root MuiButton-root MuiButton-text MuiButton-textInfo MuiButton-sizeMedium MuiButton-textSizeMedium MuiButton-colorInfo MuiButton-disableElevation MuiButton-root MuiButton-text MuiButton-textInfo MuiButton-sizeMedium MuiButton-textSizeMedium MuiButton-colorInfo MuiButton-disableElevation mui-style-ppljvl')
...
def main():
    ...
    """
    site_url = "https://www.anibis.ch"
    base_url = f"{site_url}/fr/q/telephones-mobiles"
    liste_url = f"{base_url}/Ak6ZpUGhvbmWqY2VsbFBob25lc5STkqljb21wYW55QWSncHJpdmF0ZZKobGFuZ3VhZ2WiZnKSp29yZ2FuaWOmYW5pYmlzwMDA?sorting=expensive&query=iPhone&page="

    #pages = get_page(liste_url)
    #save_pages(pages, "data_anibis_iphone")
    
    results = parse_pages("data_anibis_iphone")
    results.to_csv("liste_resutlts_anibis_iphone.csv", index=False)
    logging.info("PRINT_RESULTS -> ")
    logging.info(results)
    
    annonces = parse_content("data_anibis_iphone")
    annonces.to_csv("liste_annonces_anibis_iphone.csv", index=False)
    logging.info("PRINT_ANNONCES -> ")
