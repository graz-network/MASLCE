...
\definecolor{textgray}{HTML}{56616B}
\definecolor{softline}{HTML}{D7E2EE}

\hypersetup{
  pdftitle={Un modèle actionnable pour la détection proactive de la fraude dans les petites annonces en ligne},
  pdfauthor={Graz Network}
}

\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\footnotesize\color{textgray}Détection proactive de la fraude dans les petites annonces}
\fancyhead[R]{\footnotesize\color{textgray}Graz Network}
...
\newcommand{\callout}[1]{%
  \begin{quote}\small\textbf{Point clé.} #1\end{quote}}

\begin{document}
\thispagestyle{plain}

\begin{center}
{\LARGE\bfseries Un modèle actionnable pour la détection proactive de la fraude dans les petites annonces en ligne\par}
\vspace{0.15em}
{\large Une méthode transparente, auditable et opérationnellement pertinente pour l'identification des annonces à risque élevé\par}
\vspace{0.6em}
{\normalsize Graz Network\par}
\vspace{0.25em}
...
\noindent\textbf{Mots-clés :} détection de fraude ; petites annonces en ligne ; triage du risque ; criminalité économique ; cybercriminalité ; savoirs actionnables ; auditabilité ; détection proactive
